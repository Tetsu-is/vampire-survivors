...
*   **機能**: プレイヤーの移動操作に「移動しっぱなしモード」を追加する。
*   **操作**:
    *   特定のキー（例: `LShift`キー）を押すことで、移動しっぱなしモードのオン/オフを切り替える（トグル）。
*   **挙動**:
    *   **モードオフ時**: 通常のWASDキーまたは方向キー操作。押している間だけ移動する。
    *   **モードオン時**:
        *   WASDキーまたは方向キーで方向を入力すると、その方向に移動を開始する。
        *   キーを離しても、移動は継続される。
        *   移動は、画面の端（壁）に到達するか、別の方向キーが入力されるまで継続する。
        *   別の方向キーが入力された場合、新しい方向へ移動を開始する。
...
*   **弾丸発射レートアップ**: プレイヤーの手動攻撃（スペースキー）の発射レートが上昇する。
    *   HP上限アップ
    *   回復量アップ
    *   防御力アップ
    *   経験値獲得量アップ
